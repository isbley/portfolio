...
# 데이터 준비 --------------------------
# 파일 불러오기
apart = pd.read_csv("아파트매매가격.csv", encoding="EUC-KR", index_col=0)
# print(apart.head())

# 지역 인덱스 공백 제거
apart.index = apart.index.str.replace(' ', '')
# print(apart.head()) 팔요없는 코드지만 일단 냅두기
# print(apart.columns)
...
print(apart)
# apart.to_csv('apartjuncheori.csv', encoding='EUC-KR')


# 데이터 탐색 --------------------------
# 정보 및 결측치 확인
apart.info()
apartnan = apart.isna().sum()   # head 5개 0, tail 5개 0으로만 나옴
print(apartnan)

# 아파트 매매 가격 통계 확인
print(apart.describe())
# apart.describe().to_csv('apartdescribe.csv', encoding='EUC-KR')

# 아파트 연도별로 그룹화하기(연도 합쳐짐 + 평균값)
apartgroup = round(apart.groupby(apart.index).mean(), 3)
print(apartgroup)
# apartgroup.to_csv('apartgroup.csv', encoding='EUC-KR')
